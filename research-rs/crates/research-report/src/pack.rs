...
use research_core::types::{PackFileEntry, PackManifest, ReportStatus};
...
pub fn pack_run(folder: &RunFolder, ticker: &str) -> Result<std::path::PathBuf> {
    ...
    collect_files(&folder.root, &folder.root, &mut files, &mut file_entries)?;
...
fn collect_files(
    ...
    file_entries: &mut Vec<PackFileEntry>,
) -> Result<()> {
    for entry in fs::read_dir(path)? {
        let entry = entry?;
        let p = entry.path();
        if p.is_dir() {
            ...
            collect_files(base, &p, files, file_entries)?;
        } else {
            let rel = p.strip_prefix(base)?.to_string_lossy().replace('\\', "/");
            let bytes = fs::read(&p)?;
            files.push(rel.clone());
            file_entries.push(PackFileEntry {
                path: rel,
                size_bytes: bytes.len() as u64,
                sha256: digest_str(&String::from_utf8_lossy(&bytes)),
            });
        }
    }
    Ok(())
